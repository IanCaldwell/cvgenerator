...
class SkillsController extends Controller
{
	...
	public function actionCreate()
	{
		...
		// $this->performAjaxValidation($model);
                ...
                $userSkill=new UserJoinSkill;                

		if(isset($_POST['Skills']))
		{
			$model->attributes=$_POST['Skills'];
			if($model->save()){                            
                                $userSkill->cvgen_skill_id=$model->skill_id;
                                $userSkill->cvgen_user_id=Yii::app()->user->id;
                                $userSkill->save();
				$this->redirect(array('profile/view#tab_2'));
                        }
		}
	...
	{
            $join= UserJoinSkill::model()->findByAttributes(array('cvgen_skill_id'=>$id,'cvgen_user_id'=>Yii::app()->user->id));
            $join->delete();
            $model=$this->loadModel($id);
